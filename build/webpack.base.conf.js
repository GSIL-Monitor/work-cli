...
var browsers = ['> 1%', 'last 2 versions', 'ie >= 8', 'iOS 7', 'Firefox < 10'];
...
if (__DEV__) {
    ...
    postcssPlugins.push(
        autoprefixer({
            browsers: ['> 1%', 'last 2 versions', 'ie >= 8', 'iOS 7', 'Firefox < 10']
        })
    )
}
var config = {
    entry: {
        index: relative('src/index.js')
    },
    output: {
    ...
    module: {
        rules: [{ // 在编译之前执行这个loader，如果报错了就不继续
                enforce: 'pre',
                test: /.(js|vue)$/,
                loader: 'eslint-loader',
                include: relative('src'),
                options: {
                    configFile: relative('.eslintrc'),
                    ignoreFile: relative('.eslintignore'),
                }
            },
            {
                test: /\.js?$/,
            ...
            },
            {
                ...
                use: [{
                    loader: "vue-loader",
                    options: {
                        preserveWhitespace: false, //消除标签换行空白内容
                        transformToRequire: {
                            avatar: ['src'] //使vue使用src时,无需require引用文件
                        },
                        ...
                        loaders: {
                            ...
                            // less: 'vue-style-loader!css!happypack/loader?id=happyless'
                        }
                    }
                }],
            },
    ...
    plugins: plugins.concat([
        ...
        new HtmlWebpackPlugin({
            multihtmlCache: true,
            filename: 'index.html',
            template: relative('src/templates/index.html'),
